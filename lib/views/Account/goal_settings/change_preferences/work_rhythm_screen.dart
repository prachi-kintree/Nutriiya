...
class WorkRhythmScreen extends StatefulWidget {
  const WorkRhythmScreen({
    super.key,
  });

  @override
  State<WorkRhythmScreen> createState() => _DietaryPreferencesState();
}

class _DietaryPreferencesState extends State<WorkRhythmScreen> {
  List<SelectableFieldItem> regionalFoodPrefs = [
    SelectableFieldItem(
        icon: svgNorthIndian, dietType: " 9-to-5", isSelected: false),
    SelectableFieldItem(
        icon: svgSouthIndian, dietType: "Shift work", isSelected: false),
    SelectableFieldItem(
        icon: svgEastIndian, dietType: "Night shifts", isSelected: false),
    SelectableFieldItem(
        icon: svgCentralIndian, dietType: "Flexible hours", isSelected: false),
    SelectableFieldItem(
        icon: svgWestIndian, dietType: "Something else", isSelected: false),
  ];
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: const CommonAppBar(
        appBarTitle: "What’s Your Work Rhythm?",
      ),
      body: Container(
        height: 1.sh,
